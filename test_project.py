...
class TestMovieData(unittest.TestCase):
	...
	def test_get_movie_budget(self):
		"""Tests that the get_movie_budget method returns a unicode repr of the budget. 
		   Also tests that the budget for the chosen movie is as expected."""

		self.assertTrue(isinstance(self.budget_string, unicode))
		self.assertEqual(self.budget_string, u'$19 million')

	def test_split_budget_text(self):
		"""Tests that the split_budget_text method returns either a tuple or u'N/A'.
		   Also tests that the budget tuple for the chosen movie is as expected."""
	...
	def test_convert_split_budget_to_float(self):
		"""Tests that the convert_budget_to_float method returns either a float or u'N/A'.
		   Also tests that the budget float for the chosen movie is as expected."""
...
class TestBestPictureData(unittest.TestCase):
	...
	def test_convert_li_to_movie_data(self):
		"""Tests that the convert_li_to_movie_data converts a BeautifulSoup
		   li object for a movie to a tuple of expected data."""
	...
	def test_get_bp_movie_list_generator(self):
		...
		self.assertTrue(isinstance(self.bp_data_obj.get_bp_movie_list_generator(), types.GeneratorType))
	...
	def test_bp_movie_list_length(self):
		"""Tests that there are the correct number of movies provided in the generator.
		   This is a specific test, but only has to change once a year."""
	...
	def test_get_average_budget(self):
		"""Tests that the get_average_budget method works as expected, and ignores
		   any unavailable budgets. Uses mock named tuples for speed."""
